...
colvars_cdfw <- colvars_cdfw %>%
  select(-IndRur_0t1, -DisBog_4t5, -north6, -south7, 
       -west8, -east9, -ViolInd_1011, -DisMer_13, -RulPar_15t16) %>%
...
df_select <- colvars_cdfw %>%
  group_by(year) %>%
  dplyr::summarize(
    across(2:16, ~mean(., na.rm = TRUE)))
...
#---- Write to rds----
write_rds(colvars_cdf, file = "G:/Shared drives/snvdem/snvdem-col/data/panel/05_geocoded_panel/v1/CDF_select.rds")
