...
# 4. Print results
print(paste("Full Dataset Spearman Rho:", round(full_cor, 3)))
print(paste("Extreme Rural Subset Spearman Rho:", round(extreme_cor, 3)))
...
## REVISION 3 ----
# Check 1: Identify which years are missing weight data
missing_years <- Indices %>%
  dplyr::group_by(year) %>%
  dplyr::summarize(
...
# Check 2: Identify if specific predictors have NAs
...
write_rds(Indices, file = "G:/Shared drives/snvdem/snvdem-col/data/panel/08_final_snvdem_data/MC/Indices.rds")


# Constructing each index: FF elections
Indices_both <- Indices %>%
  mutate(snelect = (avg0t1hi*el_Urban+avg0t1lo*el_Rural+
                      avg10t11*wt_el_1011+
                      avg12*el_Sparse_population+
                      avg13*el_Remote+
                      avg14*(1-el_Indigenous)+
           ...
           (el_Urban+el_Rural+
              wt_el_1011+
              el_Sparse_population+
              el_Remote+
              (1-el_Indigenous)+
              (1-el_Ruling_party_strong)+(1-el_Ruling_party_weak)+
              el_More_development+el_Less_development+
              (1-el_Inside_capital)+(1-el_Outside_capital)
            +el_North+el_South+el_West+el_East), 
         sncivlib = (avg0t1hi*cl_Urban+avg0t1lo*cl_Rural+
                       avg10t11*wt_cl_1011+
                       avg12*cl_Sparse_population+
                       avg13*cl_Remote+
                       avg14*(1-cl_Indigenous)+
           ...
           (el_Urban+el_Rural+
              ...
              el_Remote+
              (1-el_Indigenous)+
              (1-el_Ruling_party_strong)+
              (1-el_Ruling_party_weak)+
              el_More_development+
              el_Less_development+
              (1-el_Inside_capital)+
              (1-el_Outside_capital)+
              el_North+el_South+el_West+el_East))

# write_rds(Indices_both, path = "G:/Shared drives/snvdem/snvdem-col/data/panel/08_final_snvdem_data/MC/Indices_both.rds")


# REVISION 3 ----

# 1. Ensure the merge is correct (assuming predict_hilo and weights_col exist)
Indices2 <- predict_hilo %>%
  left_join(weights_col, by = "year")

# 2. Constructing the Indices with distinct denominators
Indices_both2 <- Indices2 %>%
  mutate(
...
# 3. Save the result
write_rds(Indices_both2, "G:/Shared drives/snvdem/snvdem-col/data/panel/08_final_snvdem_data/MC/Indices_both2.rds")
...
comparison <- data.frame(
  Colleague_Mean = mean(Indices_both$sncivlib, na.rm = TRUE),
  Suggested_Mean = mean(Indices_both2$sncivlib, na.rm = TRUE)
)
print(comparison)
...
# Visual exploration
ggplot(Indices_both2, aes(x = snelect, y = sncivlib)) +
  geom_point(alpha = .1) +
  theme_light()
...
Indices_both2 <- Indices_both2 %>%
  mutate(sndem = 0.5*(snelect + sncivlib))

write_rds(Indices_both2, "G:/Shared drives/snvdem/snvdem-col/data/panel/08_final_snvdem_data/MC/Indices_both_tentative.rds")
