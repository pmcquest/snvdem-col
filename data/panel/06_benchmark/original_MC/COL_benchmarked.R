...
snclplot<- ggplot(CLbench, aes(x = as.factor(year), y = CL_col_mt)) +
  ...
  theme_light()
ggsave(snclplot, filename = "G:/Shared drives/snvdem/snvdem-col/data/panel/08_benchmark/snclplot.png", device = "png", height = 8, width = 12, units = "in", dpi = 300)
...
snelplot <- ggplot(CLbench, aes(x = as.factor(year), y = EL_col_mt)) +
  ...
  theme_light()
ggsave(snelplot, filename = "G:/Shared drives/snvdem/snvdem-col/data/panel/08_benchmark/snelplot.png", device = "png", height = 8, width = 12, units = "in", dpi = 300)
