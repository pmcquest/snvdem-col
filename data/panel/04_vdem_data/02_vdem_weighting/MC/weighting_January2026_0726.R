#---- Step 6: Subset V-Dem data ----

# Step 1: Wrangle raw data (Folder: "01_raw_data")
# Step 2: clean raw data ("02_cleaned_data")
# Step 3: Impute missing values ("03_imputation_scripts")
# Step 4: Compile and revise imputed data ("04_imputed_intermediate")
# Step 5: Calculate averages of Empirical CDF data  ("05_geocoded_panel")
# Step 6 (this step): Subset V-Dem data, calculate criteria averages, apply national range ("06_vdem_data")
# Step 7: Weight Averaged CDF data by V-Dem data ("07_weighting")
# Step 8: Benchmark using national V-Dem data ("08_benchmark")
# Step 9: Revise final snvdem index ("09_final_snvdem_data")

library(tidyverse)

# Original (MC):
# v15cl <- read.csv("C:/Users/mcoppedg/Dropbox/MC/VDemFiles/Archive/V15/V-Dem-Coder-Level-v15_csv/Coder-Level-Dataset-v15.csv")
...
# Alternatively: Install latest version of VDem (v15, as of 12/28/2025)
...
# Tangential: getting non-absolute differences
# Calculate the difference in weights (more - less)
mvl_el <- more_vs_less %>%
  mutate(weight_diff = weight_more - weight_less) #Caution: not absolute value

order <- mvl_el%>%
  group_by(dimension) %>%
  summarize(rank = mean(weight_diff, na.rm = TRUE))

mvl_el <- merge(mvl_el, order, by = "dimension", all.x = TRUE)
...
colnames(mvl_el_wide) <- c("country_text_id", "year", "Civil_unrest", "East", "Illicit_activity", "Indigenous", "Inside_capital", "Less_development", "Longer_foreign_rule", "More_development", "No_foreign_rule", "None_of_the_above", "North", "Outside_capital", "Recent_foreign_rule", "Remote", "Ruling_party_strong", "Ruling_party_weak", "Rural", "Shorter_foreign_rule", "South", "Sparse_population", "Urban", "West")

write_dta(mvl_el_wide, path = "more_vs_less_el.dta")

# Now resume the correct calculation: absolute differences
# Calculate the difference in weights (more - less)
more_vs_less <- more_vs_less %>%
  mutate(weight_diff = abs(weight_more - weight_less)) #New: made this absolute value
...
ggplot(filter(more_vs_less, country_text_id %in% selected & year>1999 & !str_detect(dimension, "foreign")),
...
# Subnational civil liberties weaker

#### Expert weights for relevance criteria
# Use v2clrgwkch and v2clrgstch in v15cl
#table(v15cl$v2clrgwkch_1)
snwkcc_0 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_0)) %>%
  group_by(country_text_id, year, v2clrgwkch_0) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_0,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_0_1 = "1",
         v2clrgwkch_0_0 = "0") 

snwkcc_1 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_1)) %>%
  group_by(country_text_id, year, v2clrgwkch_1) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_1,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_1_1 = "1",
         v2clrgwkch_1_0 = "0") 

snwkcc_2 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_2)) %>%
  group_by(country_text_id, year, v2clrgwkch_2) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_2,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_2_1 = "1",
         v2clrgwkch_2_0 = "0")

snwkcc_3 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_3)) %>%
  group_by(country_text_id, year, v2clrgwkch_3) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_3,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_3_1 = "1",
         v2clrgwkch_3_0 = "0")

snwkcc_4 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_4)) %>%
  group_by(country_text_id, year, v2clrgwkch_4) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_4,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_4_1 = "1",
         v2clrgwkch_4_0 = "0")

snwkcc_5 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_5)) %>%
  group_by(country_text_id, year, v2clrgwkch_5) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_5,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_5_1 = "1",
         v2clrgwkch_5_0 = "0")

snwkcc_6 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_6)) %>%
  group_by(country_text_id, year, v2clrgwkch_6) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_6,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_6_1 = "1",
         v2clrgwkch_6_0 = "0")

snwkcc_7 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_7)) %>%
  group_by(country_text_id, year, v2clrgwkch_7) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_7,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_7_1 = "1",
         v2clrgwkch_7_0 = "0")

snwkcc_8 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_8)) %>%
  group_by(country_text_id, year, v2clrgwkch_8) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_8,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_8_1 = "1",
         v2clrgwkch_8_0 = "0")

snwkcc_9 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_9)) %>%
  group_by(country_text_id, year, v2clrgwkch_9) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_9,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_9_1 = "1",
         v2clrgwkch_9_0 = "0")

snwkcc_10 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_10)) %>%
  group_by(country_text_id, year, v2clrgwkch_10) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_10,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_10_1 = "1",
         v2clrgwkch_10_0 = "0")

snwkcc_11 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_11)) %>%
  group_by(country_text_id, year, v2clrgwkch_11) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_11,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_11_1 = "1",
         v2clrgwkch_11_0 = "0")

snwkcc_12 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_12)) %>%
  group_by(country_text_id, year, v2clrgwkch_12) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_12,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_12_1 = "1",
         v2clrgwkch_12_0 = "0")

snwkcc_13 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_13)) %>%
  group_by(country_text_id, year, v2clrgwkch_13) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_13,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_13_1 = "1",
         v2clrgwkch_13_0 = "0")

snwkcc_14 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_14)) %>%
  group_by(country_text_id, year, v2clrgwkch_14) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_14,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_14_1 = "1",
         v2clrgwkch_14_0 = "0")

snwkcc_15 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_15)) %>%
  group_by(country_text_id, year, v2clrgwkch_15) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_15,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_15_1 = "1",
         v2clrgwkch_15_0 = "0")

snwkcc_16 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_16)) %>%
  group_by(country_text_id, year, v2clrgwkch_16) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_16,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_16_1 = "1",
         v2clrgwkch_16_0 = "0")

snwkcc_17 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_17)) %>%
  group_by(country_text_id, year, v2clrgwkch_17) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_17,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_17_1 = "1",
         v2clrgwkch_17_0 = "0")

snwkcc_18 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_18)) %>%
  group_by(country_text_id, year, v2clrgwkch_18) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_18,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_18_1 = "1",
         v2clrgwkch_18_0 = "0")

snwkcc_19 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_19)) %>%
  group_by(country_text_id, year, v2clrgwkch_19) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_19,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_19_1 = "1",
         v2clrgwkch_19_0 = "0")

snwkcc_20 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_20)) %>%
  group_by(country_text_id, year, v2clrgwkch_20) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_20,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_20_1 = "1",
         v2clrgwkch_20_0 = "0")

snwkcc_21 <- filter(v15cl, year>1899 & !is.na(v2clrgwkch_21)) %>%
  group_by(country_text_id, year, v2clrgwkch_21) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgwkch_21,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgwkch_21_1 = "1",
         v2clrgwkch_21_0 = "0")

# Merge all these dfs
snwkcc_all <- as.data.frame(cbind(snwkcc_0[,3:4], snwkcc_1, snwkcc_2[,3:4], snwkcc_3[,3:4], snwkcc_4[,3:4], snwkcc_5[,3:4], 
                                  snwkcc_6[,3:4], snwkcc_7[,3:4], snwkcc_8[,3:4], snwkcc_9[,3:4], snwkcc_10[,3:4], 
                                  snwkcc_11[,3:4], snwkcc_12[,3:4], snwkcc_13[,3:4], snwkcc_14[,3:4], snwkcc_15[,3:4], 
                                  snwkcc_16[,3:4], snwkcc_17[,3:4], snwkcc_18[,3:4], snwkcc_19[,3:4], snwkcc_20[,3:4], snwkcc_21[,3:4]))
# Reordering columns
snwkcc_all <- select(snwkcc_all, country_text_id, year, v2clrgwkch_0_0, v2clrgwkch_0_1:last_col())


# Merge in the frequencies of coders who said sn elections were the same subnationally
snwkcc_alll <- merge(snlsffHPD[,1:3], snwkcc_all, by = c("country_text_id", "year"), 
                     all.x = TRUE)   

# Calculate proportions of CEs who chose each criterion
cnc_proportions <- snwkcc_alll %>%
  mutate(pr_0 = v2clrgwkch_0_1/(v2clrgwkch_0_1 + v2clrgwkch_0_0 + snlsff_2),
         pr_1 = v2clrgwkch_1_1/(v2clrgwkch_1_1 + v2clrgwkch_1_0 + snlsff_2),
         pr_2 = v2clrgwkch_2_1/(v2clrgwkch_2_1 + v2clrgwkch_1_0 + snlsff_2),
         pr_3 = v2clrgwkch_3_1/(v2clrgwkch_3_1 + v2clrgwkch_1_0 + snlsff_2),
         pr_4 = v2clrgwkch_4_1/(v2clrgwkch_4_1 + v2clrgwkch_1_0 + snlsff_2),
         pr_6 = v2clrgwkch_6_1/(v2clrgwkch_6_1 + v2clrgwkch_6_0 + snlsff_2),
         pr_5 = v2clrgwkch_5_1/(v2clrgwkch_5_1 + v2clrgwkch_5_0 + snlsff_2),
         pr_7 = v2clrgwkch_7_1/(v2clrgwkch_7_1 + v2clrgwkch_7_0 + snlsff_2),
         pr_8 = v2clrgwkch_8_1/(v2clrgwkch_8_1 + v2clrgwkch_8_0 + snlsff_2),
         pr_9 = v2clrgwkch_9_1/(v2clrgwkch_9_1 + v2clrgwkch_9_0 + snlsff_2),
         pr_10 = v2clrgwkch_10_1/(v2clrgwkch_10_1 + v2clrgwkch_10_0 + snlsff_2),
         pr_11 = v2clrgwkch_11_1/(v2clrgwkch_11_1 + v2clrgwkch_11_0 + snlsff_2),
         pr_12 = v2clrgwkch_12_1/(v2clrgwkch_12_1 + v2clrgwkch_12_0 + snlsff_2),
         pr_13 = v2clrgwkch_13_1/(v2clrgwkch_13_1 + v2clrgwkch_13_0 + snlsff_2),
         pr_14 = v2clrgwkch_14_1/(v2clrgwkch_14_1 + v2clrgwkch_14_0 + snlsff_2),
         pr_15 = v2clrgwkch_15_1/(v2clrgwkch_15_1 + v2clrgwkch_15_0 + snlsff_2),
         pr_16 = v2clrgwkch_16_1/(v2clrgwkch_16_1 + v2clrgwkch_16_0 + snlsff_2),
         pr_17 = v2clrgwkch_17_1/(v2clrgwkch_17_1 + v2clrgwkch_17_0 + snlsff_2),
         pr_18 = v2clrgwkch_18_1/(v2clrgwkch_18_1 + v2clrgwkch_18_0 + snlsff_2),
         pr_19 = v2clrgwkch_19_1/(v2clrgwkch_19_1 + v2clrgwkch_19_0 + snlsff_2),
         pr_20 = v2clrgwkch_20_1/(v2clrgwkch_20_1 + v2clrgwkch_20_0 + snlsff_2),
         pr_21 = v2clrgwkch_21_1/(v2clrgwkch_21_1 + v2clrgwkch_21_0 + snlsff_2))
cnc_proportions <- cnc_proportions[c(1:2, 48:69)]

# Full sample
colnames(cnc_proportions) <- c("country_text_id", "year", "Rural", "Urban", "Less development", "More development", "Inside capital", "Outside capital", "North", "South", "West", "East", "Civil unrest", "Illicit activity", "Sparse population", "Remote", "Indigenous", "Ruling party strong", "Ruling party weak", "Longer foreign rule", "Shorter foreign rule", "Recent foreign rule", "No foreign rule", "None of the above")

cn_table_long <- filter(cnc_proportions[,c(1:2, 3:24)], !is.na(Rural)) %>%
  pivot_longer(cols = !c(country_text_id, year),
               names_to = "dimension",
               values_to = "weight")
...
#### Subnational civil liberties stronger
#### 
#table(v15cl$v2clrgstch_1)
snstcc_0 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_0)) %>%
  group_by(country_text_id, year, v2clrgstch_0) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_0,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_0_1 = "1",
         v2clrgstch_0_0 = "0") 

snstcc_1 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_1)) %>%
  group_by(country_text_id, year, v2clrgstch_1) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_1,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_1_1 = "1",
         v2clrgstch_1_0 = "0") 

snstcc_2 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_2)) %>%
  group_by(country_text_id, year, v2clrgstch_2) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_2,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_2_1 = "1",
         v2clrgstch_2_0 = "0")

snstcc_3 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_3)) %>%
  group_by(country_text_id, year, v2clrgstch_3) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_3,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_3_1 = "1",
         v2clrgstch_3_0 = "0")

snstcc_4 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_4)) %>%
  group_by(country_text_id, year, v2clrgstch_4) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_4,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_4_1 = "1",
         v2clrgstch_4_0 = "0")

snstcc_5 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_5)) %>%
  group_by(country_text_id, year, v2clrgstch_5) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_5,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_5_1 = "1",
         v2clrgstch_5_0 = "0")

snstcc_6 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_6)) %>%
  group_by(country_text_id, year, v2clrgstch_6) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_6,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_6_1 = "1",
         v2clrgstch_6_0 = "0")

snstcc_7 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_7)) %>%
  group_by(country_text_id, year, v2clrgstch_7) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_7,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_7_1 = "1",
         v2clrgstch_7_0 = "0")

snstcc_8 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_8)) %>%
  group_by(country_text_id, year, v2clrgstch_8) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_8,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_8_1 = "1",
         v2clrgstch_8_0 = "0")

snstcc_9 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_9)) %>%
  group_by(country_text_id, year, v2clrgstch_9) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_9,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_9_1 = "1",
         v2clrgstch_9_0 = "0")

snstcc_10 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_10)) %>%
  group_by(country_text_id, year, v2clrgstch_10) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_10,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_10_1 = "1",
         v2clrgstch_10_0 = "0")

snstcc_11 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_11)) %>%
  group_by(country_text_id, year, v2clrgstch_11) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_11,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_11_1 = "1",
         v2clrgstch_11_0 = "0")

snstcc_12 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_12)) %>%
  group_by(country_text_id, year, v2clrgstch_12) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_12,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_12_1 = "1",
         v2clrgstch_12_0 = "0")

snstcc_13 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_13)) %>%
  group_by(country_text_id, year, v2clrgstch_13) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_13,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_13_1 = "1",
         v2clrgstch_13_0 = "0")

snstcc_14 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_14)) %>%
  group_by(country_text_id, year, v2clrgstch_14) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_14,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_14_1 = "1",
         v2clrgstch_14_0 = "0")

snstcc_15 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_15)) %>%
  group_by(country_text_id, year, v2clrgstch_15) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_15,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_15_1 = "1",
         v2clrgstch_15_0 = "0")

snstcc_16 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_16)) %>%
  group_by(country_text_id, year, v2clrgstch_16) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_16,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_16_1 = "1",
         v2clrgstch_16_0 = "0")

snstcc_17 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_17)) %>%
  group_by(country_text_id, year, v2clrgstch_17) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_17,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_17_1 = "1",
         v2clrgstch_17_0 = "0")

snstcc_18 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_18)) %>%
  group_by(country_text_id, year, v2clrgstch_18) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_18,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_18_1 = "1",
         v2clrgstch_18_0 = "0")

snstcc_19 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_19)) %>%
  group_by(country_text_id, year, v2clrgstch_19) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_19,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_19_1 = "1",
         v2clrgstch_19_0 = "0")

snstcc_20 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_20)) %>%
  group_by(country_text_id, year, v2clrgstch_20) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_20,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_20_1 = "1",
         v2clrgstch_20_0 = "0")

snstcc_21 <- filter(v15cl, year>1899 & !is.na(v2clrgstch_21)) %>%
  group_by(country_text_id, year, v2clrgstch_21) %>%
  summarise(freq = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = v2clrgstch_21,
    values_from = freq,
    values_fill = list(freq = 0)) %>%
  rename(v2clrgstch_21_1 = "1",
         v2clrgstch_21_0 = "0")

# Merge all these dfs
snstcc_all <- as.data.frame(cbind(snstcc_0[,3:4], snstcc_1, snstcc_2[,3:4], snstcc_3[,3:4], snstcc_4[,3:4], snstcc_5[,3:4], 
                                  snstcc_6[,3:4], snstcc_7[,3:4], snstcc_8[,3:4], snstcc_9[,3:4], snstcc_10[,3:4], 
                                  snstcc_11[,3:4], snstcc_12[,3:4], snstcc_13[,3:4], snstcc_14[,3:4], snstcc_15[,3:4], 
                                  snstcc_16[,3:4], snstcc_17[,3:4], snstcc_18[,3:4], snstcc_19[,3:4], snstcc_20[,3:4], snstcc_21[,3:4]))
# Reordering columns
snstcc_all <- select(snstcc_all, country_text_id, year, v2clrgstch_0_0, v2clrgstch_0_1:last_col())


# Merge in the frequencies of coders who said sn elections were the same subnationally
snstcc_alll <- merge(snlsffHPD[,1:3], snstcc_all, by = c("country_text_id", "year"), 
                     all.x = TRUE)   

# Calculate proportions of CEs who chose each criterion
cncm_proportions <- snstcc_alll %>%
  mutate(pr_0 = v2clrgstch_0_1/(v2clrgstch_0_1 + v2clrgstch_0_0 + snlsff_2),
         pr_1 = v2clrgstch_1_1/(v2clrgstch_1_1 + v2clrgstch_1_0 + snlsff_2),
         pr_2 = v2clrgstch_2_1/(v2clrgstch_2_1 + v2clrgstch_1_0 + snlsff_2),
         pr_3 = v2clrgstch_3_1/(v2clrgstch_3_1 + v2clrgstch_1_0 + snlsff_2),
         pr_4 = v2clrgstch_4_1/(v2clrgstch_4_1 + v2clrgstch_1_0 + snlsff_2),
         pr_6 = v2clrgstch_6_1/(v2clrgstch_6_1 + v2clrgstch_6_0 + snlsff_2),
         pr_5 = v2clrgstch_5_1/(v2clrgstch_5_1 + v2clrgstch_5_0 + snlsff_2),
         pr_7 = v2clrgstch_7_1/(v2clrgstch_7_1 + v2clrgstch_7_0 + snlsff_2),
         pr_8 = v2clrgstch_8_1/(v2clrgstch_8_1 + v2clrgstch_8_0 + snlsff_2),
         pr_9 = v2clrgstch_9_1/(v2clrgstch_9_1 + v2clrgstch_9_0 + snlsff_2),
         pr_10 = v2clrgstch_10_1/(v2clrgstch_10_1 + v2clrgstch_10_0 + snlsff_2),
         pr_11 = v2clrgstch_11_1/(v2clrgstch_11_1 + v2clrgstch_11_0 + snlsff_2),
         pr_12 = v2clrgstch_12_1/(v2clrgstch_12_1 + v2clrgstch_12_0 + snlsff_2),
         pr_13 = v2clrgstch_13_1/(v2clrgstch_13_1 + v2clrgstch_13_0 + snlsff_2),
         pr_14 = v2clrgstch_14_1/(v2clrgstch_14_1 + v2clrgstch_14_0 + snlsff_2),
         pr_15 = v2clrgstch_15_1/(v2clrgstch_15_1 + v2clrgstch_15_0 + snlsff_2),
         pr_16 = v2clrgstch_16_1/(v2clrgstch_16_1 + v2clrgstch_16_0 + snlsff_2),
         pr_17 = v2clrgstch_17_1/(v2clrgstch_17_1 + v2clrgstch_17_0 + snlsff_2),
         pr_18 = v2clrgstch_18_1/(v2clrgstch_18_1 + v2clrgstch_18_0 + snlsff_2),
         pr_19 = v2clrgstch_19_1/(v2clrgstch_19_1 + v2clrgstch_19_0 + snlsff_2),
         pr_20 = v2clrgstch_20_1/(v2clrgstch_20_1 + v2clrgstch_20_0 + snlsff_2),
         pr_21 = v2clrgstch_21_1/(v2clrgstch_21_1 + v2clrgstch_21_0 + snlsff_2))
cncm_proportions <- cncm_proportions[c(1:2, 48:69)]

# First look, 2018 only
# cn_table2018 <- select(cn_table2018, c(1:2, 48:69))
cnm_table2018 <- filter(cncm_proportions, year==2018) 
colnames(cnm_table2018) <- c("country_text_id", "year", "Rural", "Urban", "Less development", "More development", "Inside capital", "Outside capital", "North", "South", "West", "East", "Civil unrest", "Illicit activity", "Sparse population", "Remote", "Indigenous", "Ruling party strong", "Ruling party weak", "Longer foreign rule", "Shorter foreign rule", "Recent foreign rule", "No foreign rule", "None of the above")
...
# Full sample, all countries 1900-2024
#### Expert weights for relevance criteria
# Use v2clrgstch in v15cl
#table(v15cl$v2clrgstch_1)

colnames(cncm_proportions) <- c("country_text_id", "year", "Rural", "Urban", "Less development", "More development", "Inside capital", "Outside capital", "North", "South", "West", "East", "Civil unrest", "Illicit activity", "Sparse population", "Remote", "Indigenous", "Ruling party strong", "Ruling party weak", "Longer foreign rule", "Shorter foreign rule", "Recent foreign rule", "No foreign rule", "None of the above")

cncm_proportions_long <- filter(cncm_proportions[,c(1:2, 3:24)], !is.na(Rural)) %>%
  pivot_longer(cols = !c(country_text_id, year),
               names_to = "dimension",
               values_to = "weight")
...
ggplot(filter(cncm_proportions_long, !is.nan(weight)), aes(y=reorder(dimension, weight), x=weight)) +
  geom_density_ridges(scale = 2, fill="blue", alpha = .5) +
  theme_ridges(font_size=12) + 
  theme(legend.position = "none") +
  labs(title="Expert weights in full sample", 
...
# Calculate More minus Less NOTE: Change df names here to be different from those for EL
# Rename weight in the long proportions dfs
cncm_long <- cncm_proportions_long %>%
  rename(weight_more = "weight")
cnc_long <- cn_table_long %>%
  rename(weight_less = "weight")

# merge them, keeping all observations (oops, didn't rename, overwrote el version. 
# will have to merge with version exported to Stata)
more_vs_less <- merge(cncm_long, cnc_long, by = c("country_text_id", "year", "dimension"), all.y = TRUE)
...
mvl_cl <- merge(mvl_cl, order, by = "dimension", all.x = TRUE)
...
mvl_cl_wide <- mvl_cl %>%
  pivot_wider(id_cols = c(country_text_id, year),
              ...
              values_from=weight_diff) %>%
  arrange(country_text_id, year)
colnames(mvl_cl_wide) <- c("country_text_id", "year", "Civil_unrest", "East", "Illicit_activity", "Indigenous", "Inside_capital", "Less_development", "Longer_foreign_rule", "More_development", "No_foreign_rule", "None_of_the_above", "North", "Outside_capital", "Recent_foreign_rule", "Remote", "Ruling_party_strong", "Ruling_party_weak", "Rural", "Shorter_foreign_rule", "South", "Sparse_population", "Urban", "West")

write_dta(mvl_cl_wide, path = "more_vs_less_cl.dta")
...
# renamed here
write_dta(CLmore_vs_less, path = "CLmore_vs_less.dta")

CLmore_vs_less_wide <- more_vs_less %>%
  pivot_wider(id_cols = c(country_text_id, year),
              names_from=dimension,
              values_from=weight_diff) %>%
  arrange(country_text_id, year)

colnames(CLmore_vs_less_wide) <- c("country_text_id", "year", "Civil_unrest", "East", "Illicit_activity", "Indigenous", "Inside_capital", "Less_development", "Longer_foreign_rule", "More_development", "No_foreign_rule", "None_of_the_above", "North", "Outside_capital", "Recent_foreign_rule", "Remote", "Ruling_party_strong", "Ruling_party_weak", "Rural", "Shorter_foreign_rule", "South", "Sparse_population", "Urban", "West")

write_dta(CLmore_vs_less_wide, path = "CLmore_vs_less_wide.dta")
library(haven)
CLmore_vs_less_wide <- read_dta("CLmore_vs_less_wide.dta")

# Rename variables for merging
# First civil liberties df
colnames(CLmore_vs_less_wide) <- c("country_text_id", "year", "cl_Civil_unrest", "cl_East", "cl_Illicit_activity", "cl_Indigenous", "cl_Inside_capital", "cl_Less_development", "cl_Longer_foreign_rule", "cl_More_development", "cl_No_foreign_rule", "cl_None_of_the_above", "cl_North", "cl_Outside_capital", "cl_Recent_foreign_rule", "cl_Remote", "cl_Ruling_party_strong", "cl_Ruling_party_weak", "cl_Rural", "cl_Shorter_foreign_rule", "cl_South", "cl_Sparse_population", "cl_Urban", "cl_West")

# Now import the exported elections df
library(haven)
ELmore_vs_less_wide <- read_dta("more_vs_less_wide.dta")

# and rename its columns
colnames(ELmore_vs_less_wide) <- c("country_text_id", "year", "el_Civil_unrest", "el_East", "el_Illicit_activity", "el_Indigenous", "el_Inside_capital", "el_Less_development", "el_Longer_foreign_rule", "el_More_development", "el_No_foreign_rule", "el_None_of_the_above", "el_North", "el_Outside_capital", "el_Recent_foreign_rule", "el_Remote", "el_Ruling_party_strong", "el_Ruling_party_weak", "el_Rural", "el_Shorter_foreign_rule", "el_South", "el_Sparse_population", "el_Urban", "el_West")

# Now merge them
ELCLweights_wide <- merge(ELmore_vs_less_wide, CLmore_vs_less_wide, by = c("country_text_id", "year"), all = TRUE)

# Save this so we don't have to do it all over again!
write_dta(ELCLweights_wide, path = "ELCLweights_wide.dta")

# Comparing weights for Colombia
cn_table_long_col <- filter(cn_table_long, country_text_id=="COL" & year>1999)
head(cn_table_long_col, 20)
cnc_table_long_col <- filter(cn_long, country_text_id=="COL" & year>1999)
head(cnc_table_long_col, 20)

ggplot(filter(cn_table_long, !is.nan(weight)), aes(y=reorder(dimension, weight), x=weight)) +
  geom_density_ridges(scale = 2, fill="blue", alpha = .5) +
  theme_ridges(font_size=12) + 
  theme(legend.position = "none") +
  labs(title="Expert weights in Colombia, 2000-2023", 
       subtitle = "Subnational elections more free and fair",
       y="", x="", 
       caption="Distributions are of country-years. Source: V-Dem v.15") +
  theme(axis.title.x = element_text(hjust=0.5), legend.position = "none", plot.background = element_rect(fill = "white")) +
