...
d0 <- ggplot() +
  geom_sf(data = col, aes(fill = el_d0), color = "transparent", alpha = 0.5) + # check results
  ...
  theme_void()
c0 <- ggplot() +
  geom_sf(data = col, aes(fill = el_c0), color = "transparent", alpha = 0.5) + # check results
...
#clean things up for next variable
df2rm = c("ED18", "ED18_2t3", "eCDF_2t3d", "eCDF_2t3m", "PBID", "VAM18", "df2rm")
rm(list = df2rm)
```
...
```{r V18_4-5_vdem-plot, include=FALSE, message=FALSE, results='asis'}
...
# Arrange the plots side by side
arranged_plots3 <- grid.arrange(c5el + theme(plot.title = element_text(size = 10)), 
                                c5cw + theme(plot.title = element_text(size = 10)), 
                                top = "Comparison between survey question confidence levels",
                                ncol = 2)
plot(arranged_plots3)
```
...
#clean things up for next variable
df2rm = c("CEDE18", "c5el", "c5cw", "df2rm", "arranged_plots3")
rm(list = df2rm)
...
respondents to assess elections or civil liberties across four cardinal directions: North, South,
West, East. While these categories could be measured using fuzzy sets, 
where middling municipalities would have multiple memberships to
different regions, this section settles on a binary measure of
membership, grouping together pre-existing regional categories for the purposes of
demonstrating how survey questions can be layered visually.
...
Sur, and f) Pacifico.[^4] In order to create a new variable called
...
grouped, V-Dem data for responses #6-9 are simply assigned to these categories
for each subnational survey question, without any transformation.
...
An advantage to using mapping software is the use of layers. Layers can be used to produce composite images reflecting values from more than one variable. For example, the proportional values for less free and fair elections and weak civil liberties by region can produce a rough composite image of subnational democracy,. Figure 4 reproduces two sets of layers across the four regions as classified above in order to compare confidence across democratic dimensions. There are only two variables involved in each map, so a simple mean can be used to simulate how the layered values would look. Clearly, the South is considered less democratic than the North, although both proportional scales suggest that respondents' confidence in these regional variations are low: the range for the mean of respondent agreement around less free and fair elections and weaker civil liberties is `r range(col$m1_t6t9)` while the range for the mean of respondent agreement around more free and fair elections and stronger civil liberties is `r range(col$m2_t6t9)`.

```{r V18_6-9_vdem-plot, include=FALSE, message=FALSE, results='asis'}
...
# Arrange the plots side by side
arranged_plots4 <- grid.arrange(elcw6t9 + theme(plot.title = element_text(size = 10)), 
                                emcs6t9 + theme(plot.title = element_text(size = 10)), 
                                top = "Comparison of perceived subnational democracy according to region",
                                ncol = 2)
plot(arranged_plots4)
...
#clean things up for next variable
df2rm = c("reg", "elcw6t9", "emcs6t9", "arranged_plots4", "df2rm")
rm(list = df2rm)

```

*Note: The following five variables have yet to be operationalized. Below each are some initial ideas for the variables we will use.*
...
## 11: Illicit activity
- homicide rates (perhaps more relevant for urban areas than for rural areas)
- coca growth (perhaps more relevant for rural areas than for urban areas)

## 12: Sparse population density
- Census data: rural disperse population / total population (likely highly correlated with rurality index used for responses \#0-1)
...
The survey questions also ask whether elections or civil liberties vary according to "Areas where national ruling party or group is strong
/ weak" (responses \#15-16). The crudest measure for this may be a dichotomous variable
identifying areas where the president receives a majority of the vote or not. A more
variegated measure would be the percentage of votes cast for the
president-elect vs. runner-up, for example. In this case, we can use the CDF function to create a continuous variable to be interacted with V-Dem respondent data for these items in each survey question.

The measure for ruling party support used here will be the percentage of vote that
supports the ruling party. In 2018, there was a runoff
election (second round) between Gustavo Petro and Ivan Duque; Duque won, so his is the
...
(prior to 1994, as well as 2002 and 2006) would require additional coding.
...
Official voting data comes from Registraduria (RNEC), and it includes all possible vote entries: top 2 candidates, as well as "no
mark", blank, or null ballots. The data show that Duque received `r round((sum(RP18$Duque)/sum(RP18$vtotal))*100, 2)`\% of the national vote, and Petro won `r round((sum(RP18$Petro)/sum(RP18$vtotal))*100, 2)`\%. At the municipal-level, Duque's margin of victory (`r `round(mean(RP18$MOV_pct)*100, 2)`) is larger than the national average, and the standard deviation (`r round(sd(RP18$MOV_pct), 3)`) is larger than the mean, suggesting an abnormal distribution of data. For this reason, we use the empirical CDF function to merge V-Dem data accordingly. Applying this function results in normalized values between 0-1, and higher values on this scale indicate a higher margin of victory for Duque. 
...
The proportions of respondent agreement are low; none of the survey questions accrue above 0.333 agreement. Nonetheless, we can visualize how these scores are distributed across the territory, according to operationalization for ruling party support. Figure 5 shows municipal-level data reflecting the product of the empirical CDF for V-Dem data around civil liberties and presidential margin of victory. 

```{r, V18_15-16vdem-plot, message=FALSE, results='asis'}
...
# Arrange the plots side by side
arranged_plots10 <- grid.arrange(c15cs + theme(plot.title = element_text(size = 10)), 
                                c16cw + theme(plot.title = element_text(size = 10)), 
                                top = "Areas of the country where gov't officials’ respect for civil liberties is significantly stronger / weaker",
                                ncol = 2)

plot(arranged_plots10)
```
...
RP18_15t16 <- RP18 %>% 
  ...
  mutate(MV18_15t6 = round(MOV18_15t16, 5))
#Belen de Bajira (Choco) does not have an FID number, which causes problems in ArcMap when merging. This municipality was only recently inaugurated.
...
#clean things up for next variable
df2rm = c("RP18", "c15cs", "c16cw", "arranged_plots10", "df2rm")
rm(list = df2rm)
```

# Summary indices
The normalized data can be aggregated to produce a summary index for each survey question at the municipal-level. This can be useful for...

This section describes one simple method--taking the average--by which summary data can be calculated. Additional methods, such as factor analysis or principal component analysis (PCA), could be used as well. 
