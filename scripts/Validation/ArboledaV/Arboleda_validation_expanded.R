...
# Snvdem index
sndem <- readRDS("G:/Shared drives/snvdem/snvdem-col/data/panel/08_final_snvdem_data/MC/SN_Index_tentative.rds")
...
## 1. SNDEM Processing ----
sndem_rolling <- sndem %>%
  # Ensure MPIO_CDPMP is a 5-digit string
  mutate(MPIO_CDPMP = str_pad(as.character(as.numeric(MPIO_CDPMP)), 5, pad = "0"),
         year = as.numeric(year)) %>%
  # NEW: Extract Department Code from the first two digits of MPIO_CDPMP
...
arb_nominations <- Arb %>%
  # Filter out rows missing the quality score (Q_id) 
  # AND rows missing BOTH Mpio and Dept codes
  filter(!is.na(Q_id), !(is.na(MPIO_CDPMP) & is.na(DPTO_CCDGO))) %>%
  mutate(
    # Only pad MPIO_CDPMP if it actually exists to avoid "000NA" strings
    MPIO_CDPMP = ifelse(!is.na(MPIO_CDPMP), 
                        str_pad(as.character(as.numeric(MPIO_CDPMP)), 5, pad = "0"), 
                        NA),
    # Ensure Dept code is also padded to 2 digits (e.g., "5" becomes "05")
    DPTO_CCDGO = str_pad(as.character(as.numeric(DPTO_CCDGO)), 2, pad = "0"),
    year = as.numeric(year),
...
### Expand Arb data to include municipalities within departments mentioned ----
# 1. Create the Dept-to-Mpio Map
mpio_dept_map <- sndem_rolling %>%
  distinct(DPTO_CCDGO, MPIO_CDPMP)

# 2. Split Arb into Specifics and Dept-Only
arb_specifics <- arb_nominations %>% filter(!is.na(MPIO_CDPMP))
arb_dept_only <- arb_nominations %>% filter(is.na(MPIO_CDPMP) & !is.na(DPTO_CCDGO))

# 3. Expand the Dept-Only nominations
arb_expanded <- arb_dept_only %>%
  select(-MPIO_CDPMP) %>%  # Remove the NA column to allow the join to fill it
  inner_join(mpio_dept_map, by = "DPTO_CCDGO")

# 4. Recombine
arb_final_for_join <- bind_rows(arb_specifics, arb_expanded)

# 5. Join with sndem
validation_set <- arb_final_for_join %>%
  inner_join(sndem_rolling, by = c("MPIO_CDPMP", "year"))

print(paste("New match count:", nrow(validation_set)))
...
total_raw_count <- Arb %>% 
  ...
  nrow()
print(paste("Total Observations in Arb Dataset (2000-2023):", total_raw_count))
...
time_series_data_full <- validation_set %>%
...
# 3. Bar plot
pmentions2 <- ggplot(time_series_data_full, aes(x = factor(year), y = Mentions, fill = Quality)) +
...
ggsave(
  filename = "G:/Shared drives/snvdem/snvdem-col/data/panel/09_analysis_scripts/Validation/ArboledaV/expanded/Municipality_Mentions_Plot.png",
  plot = pmentions2,
  width = 10,
  height = 6,
...
mod_overall_roll <- feols(is_high_quality ~ sndem_roll_norm | Arb_id, data = validation_set, vcov = ~Arb_id)

# Model 2: Overall Index (Annual) 
# THE MAIN ROBUSTNESS CHECK - usually the strongest result
# Use the vcov = "cluster" argument
mod_overall_annual <- feols(is_high_quality ~ sndem_norm | Arb_id, data = validation_set, vcov = ~Arb_id)

# Model 3: Components (Annual Normalized)
# Tests which pillar (Elections vs Liberties) experts actually 'feel'
mod_comp_annual <- feols(is_high_quality ~ snelect_norm + sncivlib_norm | Arb_id, data = validation_set, vcov = ~Arb_id)

# Model 4: Components (Raw Asymmetric Scale)
# Tests the components without the 0-1 normalization stretching
mod_comp_raw <- feols(is_high_quality ~ snelect + sncivlib | Arb_id, data = validation_set, vcov = ~Arb_id)

# Compare results in console
etable(mod_overall_roll, mod_overall_annual, mod_comp_annual, mod_comp_raw, 
       signif.code = c("***"=0.01, "**"=0.05, "*"=0.1))

#--> The scores tend to predict a positive rating from these survey respondents. The linear probability regression suggests that moving from a lower to a higher score on the SNVDEM index raises the probability by 34% that a respondent identifies a municipality with stronger or weaker democratic features (elections and civil liberties). 
...
collapsed_data <- validation_set %>%
  group_by(Arb_id) %>%
  summarise(
    is_high_quality = first(is_high_quality),
    sndem_norm = mean(sndem_norm, na.rm = TRUE)
  )

# Run a simple regression on the collapsed data
summary(lm(is_high_quality ~ sndem_norm, data = collapsed_data))

library(modelsummary)

# Create a list of models
models <- list(
  "Rolling Index" = mod_overall_roll,
  "Annual Index"  = mod_overall_annual,
  "Components"    = mod_comp_annual
)

# Generate a publication-ready table
msummary(models, 
         stars = c('*' = .1, '**' = .05, '***' = .01),
         coef_map = c("sndem_roll_norm" = "Democracy Index (Rolling)",
                      "sndem_norm" = "Democracy Index (Annual)",
                      "sncivlib_norm" = "Civil Liberties Component",
                      "snelect_norm" = "Electoral Component"),
         title = "Linear Probability Model: Predicting High Quality Labels",
         notes = "Fixed effects included for Expert ID. Standard errors clustered by Expert ID.")
...
msummary(
  ...
  stars = c('*' = .1, '**' = .05, '***' = .01),
  coef_map = c(
    "sndem_roll_norm" = "Democracy Index (Rolling)",
    "sndem_norm"      = "Democracy Index (Annual)",
    "sncivlib_norm"   = "Civil Liberties Component",
    "snelect_norm"    = "Electoral Component"
  ),
  title = "Table 2. Linear Probability Model: Predicting High Quality Labels",
  notes = "Fixed effects included for Expert ID (Arb_id). Standard errors clustered by Expert ID.",
  # This is the line that saves the file:
  output = "G:/Shared drives/snvdem/snvdem-col/data/panel/09_analysis_scripts/Validation/ArboledaV/expanded/Regression_Results.docx"
)
