...
download_pop_projections <- function(spatial_level, start_year, end_year,
  ...
  pp_datasets <- all_datasets %>%
    dplyr::filter(
      level == spatial_level,
      grepl("sex", description, fixed = TRUE) == include_sex,
      grepl("ethnic", description, fixed = TRUE) == include_ethnic
    )
  pp_datasets <- pp_datasets %>%
    dplyr::select(dplyr::all_of(c("name", "year"))) %>%
    dplyr::group_by(year) %>%
    dplyr::mutate(
      start = as.numeric(unlist(strsplit(year, "_", fixed = TRUE))[1]),
      end = as.numeric(unlist(strsplit(year, "_", fixed = TRUE))[2])
    ) %>%
    dplyr::ungroup()
  stopifnot(
  ...
  pp_years <- pp_datasets %>%
    dplyr::rowwise() %>%
    dplyr::mutate(year = list(seq(start, end))) %>%
  ...
  for (dataset in needed_datasets) {
    ...
    pp_data_filtered <- pp_data %>% dplyr::filter(ano %in% included_years)
...
pop_proj <- download_pop_projections("municipality", 1985, 2030)
head(pop_proj)
# for ethnic groups, only data from 2018-2030 (projections) are available...
pop_proje <- download_pop_projections("municipality", 2018, 2030, include_ethnic = TRUE)

install.packages("writexl")
library(writexl)
write_xlsx(pop_proj, "G:/Shared drives/snvdem/snvdem-col/data/panel/ColOpenData/population_projections.xlsx")
write_xlsx(pop_proje, "G:/Shared drives/snvdem/snvdem-col/data/panel/ColOpenData/population_projections_ethnic2018-30.xlsx")
