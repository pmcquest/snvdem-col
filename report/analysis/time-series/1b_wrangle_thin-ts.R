#----- Wrangling time-series data for Colombia (snvdem): Thin index -----
...
# Thick index (see script "1a_wrangle_thick-ts.R"): includes variables for which ample data is available over time. This often refers to municipal-level data after 1990. 
# Thin index: mostly includes variables for which less ample data is available. This often refers to non-municipal or regional data pre-1990. 

## Current gaps:
## 0-1: rurality pre-1993? Unclear what a thin measure would be for this. Perhaps a categorical measure of urbanization (e.g., city, mid-size town, rural town, dispersed)?
## 2-3: mostly at the departmental level
## 10-11: "dummy" variables for La Violencia
## 13: Major roads only?
## 14: no census pre-1993?
## 15-16: direct elections began in 1988...

# IPUMS data may provide some data, however there may be too much "missingness" because they work with a stratified sample provided by DANE that is equivalent to 10% of all Census data. See here for 2005 Census data for example: https://international.ipums.org/international-action/sample_details/country/co#tab_co2005a.
## (0-1) rural/urban
## (2-3) socio-economic
...
# this data set also contains important descriptive information (department, province, distance to Bogota [#4-5]) that we will hang onto.
col_ts <- col_ts[c("DPTO_CCDGO", "MPIO_CDPMP", "year", "depto", "provincia", "municipio", "disbogota")]
col_ts <- col_ts %>%
  rename(DisBog_4t5 = 8)
...
## ---- 0-1 Rural and Urban (pending) ----
# IPUMS-Colombia uses a categorical measure of "urban"  ... 

##---- 2-3 Economic development ----
# We have used PBI (departmental), but there may be others. 
...
###---- Thin*: PBID (dept, 1980-2022) ----
# *this is department-level data
PBID <- read_excel("G:/Shared drives/snvdem/snvdem-col/data/panel/2-3_EconDevt/anex-PIBDep-RetropolacionDepartamento-2022pr.xlsx", 
...
###---- Thin*: Multi-dimensional poverty (2005 & 2018) ----
# we may want to drop or 
# IPM from CEDE
IPM <- IR %>%
  select(2:3|10)
...
# merge raw data
col_ts <- merge(col_ts, ED, by = c("DPTO_CCDGO", "MPIO_CDPMP", "year"), all.x = TRUE)
...
col_tse <- merge(col_tse, EDc, by = c("DPTO_CCDGO", "MPIO_CDPMP", "year"), all.x = TRUE)
...
###---- Thick/Thin: Distance to Bogota (1993-2020) ----
...
###---- Thick/Thin: Distance from center-point (ratio) ----
# rather than simply categorical data (which is arbitrary), we use continuous distances
# from Matt Sisk
...
##---- 10 Civil unrest ----
# Data on violence are tricky. Official data include reports of displacement or confinements to MinDefense, but there may be reporting issues. CEDE also offers historic dummy variables of presence of La Violencia and Land Conflicts, which are very thin. Alternatively, CEV-HRDAG in Colombia has produced estimates based on reports from hundreds of NGOs related to displacement and other features between 1985-2018. 
...
###---- Thinner: Displacement (1993-2021) and Confinements (1993-2020) ----
...
###---- **Thinnest: Violence (1948-53) and Land conflicts (1901-31) ----
# dummy variables

###---- **Thick: Forced Displacement (1985-2021) ----
# HRDAG data requires work...

##---- 11 Illicit activity ----
###---- **Thinner: Robbery (2003-2020), Homicides (2003-2020), Eradication (1998-2020) ----
...
###---- Thick: Density (1985-2030) ----

SP_12 <- read_excel("G:/Shared drives/snvdem/snvdem-col/data/panel/ColOpenData/population_projections.xlsx")
# Data for Indigenous population (#14) is only available 2018-2030, so we can use CEDE projections (1993-2017) and historical data (?)
...
###---- Thin: Distance to market (1993-2020) ----
DM = IR %>%
  select(2:3|8)
...
# Data for Indigenous population (#14) from National Census data from Los Andes Epiverse-TRACE initiative is only available 2018-2030, so need to look for historical data (?)
